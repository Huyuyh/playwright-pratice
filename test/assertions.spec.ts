...
test.only("Multiple soft assertions with final hard assertion", async ({
  page,
}) => {
  await page.goto("https://the-internet.herokuapp.com/login");
  ...
  await expect
    .soft(page.getByRole("heading", { name: "Login Page" }))
    .toBeVisible();
  ...
  expect(
    errors.length,
    `There where ${errors.length} soft assertions errors`,
  ).toBe(0);
});
